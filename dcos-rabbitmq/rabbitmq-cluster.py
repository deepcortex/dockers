...
MARATHON_URI = os.environ.get('MARATHON_URI', 'https://master.mesos:8443')
...
def get_marathon_app(app_id, authentication_token):
    response = requests.get('%s/v2/apps%s' % (MARATHON_URI, app_id), headers={'Authorization': 'token=' + authentication_token}, verify=False)
    state = response.json()
    return state['app']


def get_marathon_tasks(app_id, authentication_token):
    response = requests.get('%s/v2/apps%s/tasks' % (MARATHON_URI, app_id), headers={'Authorization': 'token=' + authentication_token}, verify=False)
    state = response.json()
    return state['tasks']


def get_node_ips(authentication_token):
    my_ip = ''
    other_ips = []
    if MARATHON_URI:
...
def wait_for_nodes_to_start(authentication_token):
    while True:
...
def run():
    ...
    authentication_token = get_authentication_token(marathon_client_marathon_username, marathon_client_marathon_password)
